 ...
 *
 * @returns La liste des pays avec leurs participations et l'état de chargement
 */
export function useOlympicData(): { data: Country[]; isLoading: boolean } {
  const [data, setData] = useState<Country[] | null>(null)
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    // Simulation d'un appel API avec un délai
    // Dans une application réelle, cela serait un appel API vers le backend
    // Par exemple:
 ...
 */
export function calculateTotalMedals(olympic: Country): number {
 ...
 */
export function calculateTotalAthletes(olympic: Country): number {
  return olympic.participations.reduce(
